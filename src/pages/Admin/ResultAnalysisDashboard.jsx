...
const ResultAnalysisDashboard = () => {
  ...
  const processChartData = (rankingsData) => {
    ...
    rankingsData.forEach(ranking => {
      ...
      const marks = ranking.marks;
      const percentage = ranking.percentage;
      const totalMarks = ranking.totalMarks || calculateTotalMarks(marks, percentage);
  ...
  // Process data for performance distribution chart
  const processPerformanceDistribution = (rankingsData) => {
    if (!Array.isArray(rankingsData) || rankingsData.length === 0) {
      return { labels: [], datasets: [] };
    }
    ...
    const performances = rankingsData.map(ranking => {
      const totalMarks = ranking.totalMarks || calculateTotalMarks(ranking.marks, ranking.percentage);
      return totalMarks > 0 ? (ranking.marks / totalMarks) * 100 : 0;
    });
    ...
    const colors = Object.keys(buckets).map((label, index) => {
      const start = parseFloat(label.split('-')[0]);
      if (start >= 80) return 'rgba(16, 185, 129, 0.8)';
      if (start >= 60) return 'rgba(34, 197, 94, 0.8)';
      if (start >= 40) return 'rgba(59, 130, 246, 0.8)';
      if (start >= 20) return 'rgba(245, 158, 11, 0.8)';
      return 'rgba(239, 68, 68, 0.8)';
    });
  ...
  // Process data for category performance comparison
  const processCategoryPerformance = (rankingsData) => {
    if (!Array.isArray(rankingsData) || rankingsData.length === 0) {
      return { labels: [], datasets: [] };
    }
  ...
  const chartOptions = {
    ...
    scales: {
      x: {
        beginAtZero: true,
        title: { display: true, text: 'Average Marks', font: { weight: 'bold', size: 14 }, color: '#004d66' },
        grid: { color: 'rgba(0, 0, 0, 0.1)', drawBorder: false },
        ticks: { color: '#666', font: { size: 12 }, callback: function(value) { return value % 1 === 0 ? value : value.toFixed(1); } }
      },
      y: {
        title: { display: true, text: 'Users', font: { weight: 'bold', size: 14 }, color: '#004d66' },
        grid: { display: false },
  ...
  const distributionChartOptions = {
    ...
    plugins: {
      ...
      tooltip: { callbacks: { label: function(context) { return `${context.label}: ${context.parsed.y} results`; } } }
    },
    scales: {
      y: { beginAtZero: true, title: { display: true, text: 'Number of Results', font: { weight: 'bold', size: 14 }, color: '#004d66' }, ticks: { stepSize: 1, precision: 0 } },
      x: { title: { display: true, text: 'Performance Range (%)', font: { weight: 'bold', size: 14 }, color: '#004d66' }, ticks: { maxRotation: 45, minRotation: 0 } }
    }
  };
  ...
  const categoryChartOptions = {
    ...
    plugins: {
      ...
      tooltip: { callbacks: { label: function(context) { const category = context.label; const performance = context.parsed.y.toFixed(1); return [`Category: ${category}`, `Average Performance: ${performance}%`]; } } }
    },
    scales: {
      y: { beginAtZero: true, max: 100, title: { display: true, text: 'Average Performance (%)', font: { weight: 'bold', size: 14 }, color: '#004d66' } },
      x: { title: { display: true, text: 'Job Categories', font: { weight: 'bold', size: 14 }, color: '#004d66' }, ticks: { maxRotation: 45, minRotation: 0, callback: function(value, index, values) { const label = this.getLabelForValue(value); return label.length > 20 ? label.substring(0, 20) + '...' : label; } } }
    }
  };
  ...
  useEffect(() => {
    ...
    const startSignalRConnection = () => {
      connection.start()
        .then(() => { console.log("SignalR Connected"); retryAttempts = 0; connection.on("ReceiveQuizResult", (result) => { console.log("New quiz result received:", result); fetchUpdatedRankings(); }); })
        .catch(err => { console.error("SignalR Connection Error: ", err); if (isMounted && retryAttempts < maxRetries) { retryAttempts++; console.log(`Attempting to reconnect SignalR (Attempt ${retryAttempts}/${maxRetries})...`); setTimeout(startSignalRConnection, Math.random() * retryDelay); } else if (isMounted) { console.error("Maximum SignalR retry attempts reached."); } });
    };

    const fetchUpdatedRankings = async () => {
      try { const response = await axios.get("https://localhost:5031/api/analysis/rankings"); const rankingsData = response.data.$values || response.data; if (isMounted) setRankings(rankingsData); } catch (error) { console.error("Error fetching updated rankings:", error); if (isMounted) setError("Failed to fetch updated rankings"); }
    };

    const fetchInitialRankings = async () => {
      try { setLoading(true); setError(null); const response = await axios.get("https://localhost:5031/api/analysis/rankings"); const rankingsData = response.data.$values || response.data; if (isMounted) { setRankings(rankingsData); setLoading(false); } } catch (error) { console.error("Error fetching initial rankings:", error); if (isMounted) { setError(`Failed to fetch rankings: ${error.message}`); setLoading(false); } }
    };
  ...
  return (
    <div className="flex">
      ...
      <div className="flex-1 transition-all duration-300 overflow-x-auto" style={{ marginLeft: isSidebarCollapsed ? '100px' : '300px' }}>
        <AdminNavbar isSidebarCollapsed={isSidebarCollapsed} />
        <div className="p-6 bg-gray-100 min-h-screen pl-8">
          <h2 className="text-2xl font-bold mb-4 text-[#004d66]">Result Analysis Dashboard</h2>
          {/* <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded">
            <p><strong>Debug Info:</strong></p>
            <p>Loading: {loading ? 'Yes' : 'No'}</p>
            <p>Error: {error || 'None'}</p>
          ...
          {loading && (<div className="bg-white p-4 rounded-lg shadow-md mt-4"><p className="text-center text-gray-600">Loading rankings...</p></div>)}
          {error && !loading && (<div className="bg-red-50 border border-red-200 p-4 rounded-lg shadow-md mt-4"><p className="text-red-600 font-semibold">Error: {error}</p></div>)}
          {!loading && !error && (
            <div className="bg-white p-4 rounded-lg shadow-md mt-8">
              <h3 className="text-xl font-semibold text-gray-700 mb-4">Quiz Result Rankings</h3>
              <div className="overflow-x-auto">
                {/* <table className="min-w-full text-left">
                  <thead><tr className="bg-[#004d66] text-white"><th className="p-2">Rank</th><th className="p-2">Name</th><th className="p-2">Marks</th><th className="p-2">Percentage (%)</th><th className="p-2">Time Taken (s)</th><th className="p-2">Category</th></tr></thead>
                  <tbody>
                    {Array.isArray(rankings) && rankings.length > 0 ? rankings.map((r, index) => (
                      <tr key={index} className={index % 2 === 0 ? "bg-gray-50" : ""}><td className="p-2">{r.rank}</td><td className="p-2">{r.name}</td><td className="p-2">{r.marks}/{r.totalMarks}</td><td className="p-2">{r.percentage.toFixed(2)}%</td><td className="p-2">{r.timeTaken}</td><td className="p-2">{r.category}</td></tr>
                    )) : <tr><td colSpan="6" className="p-4 text-center text-gray-500">No rankings data available</td></tr>}
                  </tbody>
                </table> */}
        ...
        <thead>
          <tr className="bg-[#004d66] text-white">
            ...
            <th className="p-4  border-b border-gray-200">Time Taken (s)</th>
        ...
        <tbody>
          {Array.isArray(rankings) && rankings.length > 0 ? (
            rankings.map((r, index) => (
              <tr key={index} className={index % 2 === 0 ? "bg-gray-50" : ""}>
                <td className="p-4  border-b border-gray-200">{r.rank}</td>
                <td className="p-4  border-b border-gray-200">{r.name}</td>
                ...
                <td className="p-4  border-b border-gray-200">{r.timeTaken}</td>
                <td className="p-4  border-b border-gray-200">{r.category}</td>
              </tr>
            ))
          ) : (
            <tr>
...
{/* Explicit Gap */}
      <div className="h-16"></div> {/* This creates a 4rem (64px) vertical gap */}
      {/* Or h-20 for 5rem (80px), h-24 for 6rem (96px), etc. */}
          ...
          {!loading && !error && Array.isArray(rankings) && rankings.length > 0 && (
            <div className="mt-20 bg-white p-6 rounded-lg shadow-md">
              <div className="mb-6 flex items-center space-x-4">
                <label className="text-sm font-medium text-gray-700">Filter by Job Role:</label>
                <div className="relative">
                  <select value={selectedCategory} onChange={(e) => setSelectedCategory(e.target.value)} className="px-3 py-1 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 min-w-[200px]">
                    <option value="all">All Job Roles</option>
                    {getFilteredCategories().map(category => <option key={category} value={category}>{category}</option>)}
                  </select>
                  {getAvailableCategories().length > 10 && (<div className="mt-2"><input type="text" placeholder="Search job roles..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="px-2 py-1 border border-gray-300 rounded text-xs w-full" /><p className="text-xs text-gray-500 mt-1">{getFilteredCategories().length} of {getAvailableCategories().length} roles shown</p></div>)}
                </div>
                {selectedCategory !== 'all' && <span className="text-sm text-gray-500">Showing only {selectedCategory} results for fair comparison</span>}
              </div>
              <div className="h-80 mt-8">
                <Bar data={processChartData(getFilteredRankings())} options={chartOptions} />
              </div>
              <div className="mt-6 flex justify-center space-x-6 text-sm">
                <div className="flex items-center space-x-2"><div className="w-4 h-4 bg-green-500 rounded"></div><span>High Performance (≥80%)</span></div>
                <div className="flex items-center space-x-2"><div className="w-4 h-4 bg-blue-500 rounded"></div><span>Good Performance (60-79%)</span></div>
                <div className="flex items-center space-x-2"><div className="w-4 h-4 bg-orange-500 rounded"></div><span>Average Performance (40-59%)</span></div>
                <div className="flex items-center space-x-2"><div className="w-4 h-4 bg-red-500 rounded"></div><span>Low Performance (&lt;40%)</span></div>
              </div>
            </div>
          )}
          {!loading && !error && Array.isArray(rankings) && rankings.length > 0 && (
            <div className="mt-20">
              <div className="mb-8 grid grid-cols-1 md:grid-cols-3 gap-10 mt-8">
                <div className="bg-white p-6 rounded-lg shadow-md"><h3 className="text-lg font-semibold text-gray-700 mb-2">Total Results</h3><p className="text-3xl font-bold text-blue-600">{rankings.length}</p></div>
                <div className="bg-white p-6 rounded-lg shadow-md"><h3 className="text-lg font-semibold text-gray-700 mb-2">Job Categories</h3><p className="text-3xl font-bold text-green-600">{getAvailableCategories().length}</p></div>
                <div className="bg-white p-6 rounded-lg shadow-md"><h3 className="text-lg font-semibold text-gray-700 mb-2">Active Users</h3><p className="text-3xl font-bold text-purple-600">{new Set(rankings.map(r => r.name)).size}</p></div>
              </div>
              <div className="grid grid-cols-1 xl:grid-cols-2 gap-8 mt-8">
                <div className="bg-white p-6 rounded-lg shadow-md"><div className="h-80 mt-8"><Bar data={processPerformanceDistribution(getFilteredRankings())} options={distributionChartOptions} /></div></div>
                <div className="bg-white p-6 rounded-lg shadow-md"><div className="h-80 mt-8"><Bar data={processCategoryPerformance(getFilteredRankings())} options={categoryChartOptions} /></div></div>
              </div>
              <div className="mt-8 bg-blue-50 border border-blue-200 p-6 rounded-lg">
                {/* <h4 className="font-semibold text-blue-800 mb-3">Chart Flexibility Features:</h4>
                <ul className="text-sm text-blue-700 space-y-2">
