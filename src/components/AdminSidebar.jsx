...
const AdminSidebar = ({ onToggleCollapse }) => {
  const [collapsed, setCollapsed] = useState(false);
  ...
  return (
    <div
      className={`bg-[#004d66] text-white h-screen transition-all duration-300 ${collapsed ? "w-16" : "w-64"} fixed left-0 top-0 z-50`}
    >
      <div className="p-4">
        <button onClick={toggleCollapse} className="w-full text-right text-white/70 hover:text-white">
          {collapsed ? "→" : "←"}
        </button>
      </div>
        ...
        <ul className="space-y-2 px-2">
          {navItems.map((item) => (
            <li key={item.name}>
              <a
                href={item.path}
                className={`flex items-center p-3 rounded-md transition-colors ${false ? "bg-[#003d56] text-white" : "text-white/80 hover:bg-[#003d56] hover:text-white"}`}
              >
                <item.icon className={`text-xl ${collapsed ? "mx-auto" : "mr-3"}`} />
                {!collapsed && <span>{item.name}</span>}
              </a>
